...
import (
	...
	apisCluster "github.com/intive/provider-stackit/apis/cluster"
	apisNamespaced "github.com/intive/provider-stackit/apis/namespaced"
	"github.com/intive/provider-stackit/config"
	"github.com/intive/provider-stackit/internal/clients"
	controllerCluster "github.com/intive/provider-stackit/internal/controller/cluster"
	controllerNamespaced "github.com/intive/provider-stackit/internal/controller/namespaced"
	"github.com/intive/provider-stackit/internal/features"
	"github.com/intive/provider-stackit/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for STACKIT").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-stackit"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-stackit",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped STACKIT APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced STACKIT APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped STACKIT controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced STACKIT controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped STACKIT controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced STACKIT controllers")
	}
