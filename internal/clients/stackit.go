...
import (
	...
	clusterv1beta1 "github.com/intive/provider-stackit/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/intive/provider-stackit/apis/namespaced/v1beta1"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if v, ok := creds[defaultRegion]; ok {
			ps.Configuration[defaultRegion] = v
		}
		if v, ok := creds[serviceAccountKey]; ok {
			ps.Configuration[serviceAccountKey] = v
		}
		return ps, nil
	}
}
